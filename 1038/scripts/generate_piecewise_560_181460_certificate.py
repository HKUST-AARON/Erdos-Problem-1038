...
def main():
    cert_path = 'piecewise_five_atom_181460_560blocks_margin_tuned_candidate.json'
    ...
    over = gap_worst([float(x) for x in cert['blocks'][0]['weights']], shifts,
                     float(cert['blocks'][0]['A']), float(cert['blocks'][0]['C']))
    # find true gap worst across all blocks
