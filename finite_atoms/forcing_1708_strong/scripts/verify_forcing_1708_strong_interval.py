#!/usr/bin/env python3
"""Finite interval-box verifier for the stronger (-1.708, 0) forcing family.

This verifies the stronger two-parameter forcing claim needed by the
`M = 1.814600` finite-atom route:

    a in [-1.708, -sqrt(2)],  b in [0, 1.836 + a],
    w = 1.395 - b,          c = 1.071 - b,
...
The script covers the parameter triangle by the rectangle
    ...
    a in [-1.708, -sqrt(2)],  s in [0, 1],  b = s(1.836+a),
...
def export_certificate(path: Path, stats: CertStats) -> None:
    payload = {
        "format": "erdos1038.forcing_1708_strong_interval.v1",
        "precision": PRECISION,
...
def check_exported_certificate(path: Path) -> CertStats:
    ...
    if payload.get("format") != "erdos1038.forcing_1708_strong_interval.v1":
...
def main() -> int:
    ...
    print("Finite interval-box verifier for stronger forcing family")
    print(f"a-domain: [{A_MIN}, {A_MAX}]")
    print("s-domain: [0, 1], b=s(1.836+a)")
    print("x-domain: [0, 1]")
