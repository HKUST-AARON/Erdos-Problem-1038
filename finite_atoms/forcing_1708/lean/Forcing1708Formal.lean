...
/-!
# Formal constants for the `[-1.7,0]` forcing branch

This file contains the exact parameter arithmetic for the conservative forcing
branch used before the five-atom tail block.
-/

namespace Erdos1038
namespace Forcing1708
...
theorem cOf_lower {a s : ℝ} (ha : a ∈ ADomain) (hs : s ∈ SDomain) :
    q 665213562373095 1000000000000000 ≤ cOf a s := by
  have hb := bOf_upper ha hs
...
theorem forcing_domain_arithmetic_certificate {a s : ℝ}
    (ha : a ∈ ADomain) (hs : s ∈ SDomain) :
    0 ≤ bOf a s ∧
    bOf a s ≤ q 405786437626905 1000000000000000 ∧
    0 < wOf a s ∧
    q 665213562373095 1000000000000000 ≤ cOf a s ∧
    cOf a s ≤ cShift ∧
    0 < -1 - a ∧
    1 < q 2071 1000 - bOf a s := by
