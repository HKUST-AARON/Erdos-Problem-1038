...
theorem realMeasure_abs_sub_integrable
  ...
  have hconst :
      Integrable (fun _ : ℝ => |x| + 1) (realMeasure μ) :=
    integrable_const (|x| + 1)
  refine hconst.mono' (by fun_prop : AEStronglyMeasurable
    (fun t : ℝ => |x - t|) (realMeasure μ)) ?_
...
/--
Spanning-interval selected-component constructor with boundedness discharged
from the global positive-set window.  The remaining explicit inputs are the
positive spanning interval, openness, and the diagonal-exception policy.
-/
theorem exists_positiveComponent_augmentedMaximal_of_positive_spanning_interval_auto_bdd
    {μ : ProbabilityMeasure UnitInterval1038} {x δ : ℝ}
    (hopen : IsOpen (PositiveSet (unitIntervalLogPotential μ)))
    (hxbase : x ∈ Ioo (-1 : ℝ) 0)
    (hδ : 0 < δ)
    ...
    ∃ C : PositiveComponent μ,
      ...
      0 < C.right := by
  exact
    exists_positiveComponent_augmentedMaximal_of_positive_spanning_interval
      (μ := μ) (x := x) (δ := δ)
      hopen hxbase hδ hspan
      (unitInterval_positiveSet_connectedComponentIn_bddBelow μ x)
...
/--
Endpoint-data connected-component selection with right positivity.  This
combines the endpoint-lower-bound connected-component constructor with the
maximality/right-neighbourhood bridge, producing the selected component in the
shape needed downstream.
-/
theorem exists_positiveComponent_baseline_right_pos_of_endpoint_lower_bound_connectedComponentIn_zero_right
    {μ : ProbabilityMeasure UnitInterval1038} {p δ : ℝ}
    (hp : (1 / 2 : ℝ) ≤ p)
    (hendpoint :
      HasNormalizedEndpointLowerBound (unitIntervalLogPotential μ) p)
    (hopen : IsOpen (PositiveSet (unitIntervalLogPotential μ)))
    ...
    ∃ C : PositiveComponent μ,
      ...
      Ioo (-1 : ℝ) 0 ⊆ C.interval ∧
      0 < C.right := by
  rcases exists_positiveComponent_baseline_of_endpoint_lower_bound_connectedComponentIn_bdd
      (μ := μ) (p := p) hp hendpoint hopen hbddBelow hbddAbove with
    ⟨C, hmax, hbaseline, hinter⟩
  have hright_pos :
      0 < C.right :=
    C.right_pos_of_endpoint_lower_bound_intersects_baseline_intervalMaximal_zero_right
      hp hendpoint hmax hinter hδ hzero hright
...
/--
Pointwise baseline consequence of the local compact bridge: if the analytic
local hypotheses hold around a baseline point, then that point itself lies in
the truncated positive set.
-/
theorem mem_unitIntervalTruncatedPositiveSet_of_baseline_local_compact_data
    {μ : ProbabilityMeasure UnitInterval1038} {x truncε : ℝ}
    (hx : x ∈ Ioo (-1 : ℝ) 0)
    (htruncε : 0 < truncε)
    (hlocal :
      ∀ a b : ℝ,
        ...
        (∀ threshold : ℝ, 0 < threshold →
          ...
          ∀ y : ℝ, y ∈ Icc a b →
            singularTailMass truncε μ y < ENNReal.ofReal (threshold / 2))) :
    x ∈ unitIntervalTruncatedPositiveSet μ := by
  rcases exists_baseline_Icc_neighborhood_subset_truncatedPositiveSet
      hx htruncε hlocal with
    ⟨a, b, hxab, _hsub_base, hsub_trunc⟩
...
and tail control at `0`.  This removes the direct `0 ∈
unitIntervalTruncatedPositiveSet μ` input from the local-baseline component
selection path.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_zero_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
...
/--
Threshold version of the previous local-baseline component constructor.  The
zero input is now a uniform-margin statement `threshold < U(0)` plus tail
control at the same threshold, matching the compact-baseline threshold bridge.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_zero_threshold_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
...
/--
Concrete-right-neighbourhood version of the local-baseline/threshold-zero
selected-component constructor.  Instead of a global right-side diagonal policy,
it only requires one positive right neighbourhood of `0` already known to be
truncated-positive and off diagonal.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_concrete_right_zero_threshold_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
...
`Ioo 0 δ ⊆ unitIntervalTruncatedPositiveSet μ` input from the concrete-right
constructor: the right truncated positivity is produced by the same
threshold/tail bridge used elsewhere.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_right_threshold_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
  ...
  have hright :
    ...
    unitIntervalTruncatedPositiveSet_interval_subset_of_threshold_tailMass
      htruncε hthreshold hright_pos hright_no_diag hright_tail
  exact
    exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_concrete_right_zero_threshold_tail
      μ hlog_int htruncε hthreshold hbaseline_local
      hzero_pos hzero_no_diag hzero_tail hδ hright hright_no_diag

/--
Closed-right off-diagonal version of the local-baseline selected-component
constructor.  A single disjointness hypothesis on `Icc 0 δ` supplies both
the off-diagonal condition at `0` and on the right neighbourhood `Ioo 0 δ`.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_threshold_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
    ...
    ∃ C : PositiveComponent μ,
      Ioo (-1 : ℝ) 0 ⊆ C.interval ∧
      0 < C.right := by
  have hzero_no_diag : 0 ∉ diagonalAtomSet μ := by
    rw [Set.disjoint_left] at hzero_right_no_diag
    exact hzero_right_no_diag ⟨le_rfl, le_of_lt hδ⟩
  have hright_no_diag :
      Disjoint (Ioo (0 : ℝ) δ) (diagonalAtomSet μ) := by
    rw [Set.disjoint_left]
    intro x hx
    rw [Set.disjoint_left] at hzero_right_no_diag
    exact hzero_right_no_diag ⟨le_of_lt hx.1, le_of_lt hx.2⟩
  exact
    exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_right_threshold_tail
      μ hlog_int htruncε hthreshold hbaseline_local
      hzero_pos hzero_no_diag hzero_tail hδ
      hright_pos hright_no_diag hright_tail

/--
Closed-right interval threshold/tail version of selected-component
construction.  A single package of ordinary threshold positivity,
off-diagonal disjointness, and tail control on `Icc 0 δ` supplies the separate
zero and right-neighbourhood inputs.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_interval_threshold_tail
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
  ...
  exact
    exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_threshold_tail
      μ hlog_int htruncε hthreshold hbaseline_local
...
uniform threshold is extracted internally by compactness.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_compact_data
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
    ...
    ∃ C : PositiveComponent μ,
      ...
      0 < C.right := by
  rcases continuousOn_positive_Icc_exists_strict_lower_threshold
      hright_cont hright_pos with
    ⟨threshold, hthreshold, hright_threshold⟩
  exact
    exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_interval_threshold_tail
      μ hlog_int htruncε hthreshold hbaseline_local hδ
...
`Ioo (-1) 0`, rather than pointwise with the extra redundant membership data.
-/
theorem exists_positiveComponent_baseline_right_pos_of_baseline_compact_data_and_closed_right_compact_data
    (μ : ProbabilityMeasure UnitInterval1038)
    (hlog_int : ∀ x : ℝ, x ∉ diagonalAtomSet μ →
      Integrable
        (fun t : UnitInterval1038 => Real.log (1 / |x - (t : ℝ)|))
  ...
  exact
    exists_positiveComponent_baseline_right_pos_of_baseline_local_compact_data_and_closed_right_compact_data
...
/--
Support-uniqueness version of the concrete endpoint consequence.

This is the current narrowest package-level interface: once the true
variation argument supplies the selected component, its baseline placement,
right endpoint positivity, Tao boundary-average inequality, and real-support
uniqueness inside the component, all endpoint-mass bookkeeping and
endpoint-remainder kernel integrability are generated internally.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_support_unique_component_data
    (hSupportUniqueComponentDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  rcases unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_secondary_minimizer with
    ⟨μ, hPrimary, hSecondary⟩
  rcases hSupportUniqueComponentDataFromVariation μ hPrimary hSecondary with
  ...
  let Pack : TaoVariationComponentPackage (unitIntervalLogPotential μ) :=
    taoVariationComponentPackage_of_canonicalEndpointMass_support_unique_baseline_data
      μ mean_choice reflected translation C xMinus xPlus hcomponent_interval
      hbaseline hright hboundary hunique
  let D : TaoEndpointNormalizationData (unitIntervalLogPotential μ) :=
    Pack.toTaoEndpointNormalizationData
  exact ⟨μ, hPrimary, hSecondary, D.toNormalizedEndpointPotential,
    D.baseline_length_le_positiveSet⟩
...
/--
Replacement-rigidity version of the automatic-tail endpoint consequence.

Compared with
`unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_moment_rigidity_data`,
the provider no longer supplies component-block second-moment equality.  The
equality is derived internally from:

* automatic primary nonincrease for the barycenter replacement;
* secondary minimality of the selected minimizer;
* concrete second-moment nonincrease of the component replacement.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_data
    (hComponentReplacementAutoTailReplacementRigidityDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_moment_rigidity_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentReplacementAutoTailReplacementRigidityDataFromVariation
      μ hPrimary hSecondary with
    ⟨mean_choice, reflected, translation, C, R,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright,
      hboundary, hunique⟩
  ...
  exact ⟨mean_choice, reflected, translation, C, R,
    xMinus, xPlus, hcomponent_interval, hbaseline, hright, hboundary,
    hsecondMoment_eq, hunique⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_zero_neighborhood_data
    (hComponentReplacementAutoTailReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentReplacementAutoTailReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨mean_choice, reflected, translation, C, R,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright,
      hboundary, hzero⟩
  have hcomponent_open : IsOpen C.interval := by
    rw [hcomponent_interval]
    exact isOpen_Ioo
  have hunique :
      ∀ t : ℝ, t ∈ (realMeasure μ).support → t ∈ C.interval → t = -1 :=
    unique_support_in_component_of_support_neighborhood_zero
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_atomization_data
    (hComponentReplacementAutoTailReplacementRigidityAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_normalized_atomization_data
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_atomization_component_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentReplacementAutoTailReplacementRigidityNormalizedAtomizationDataFromVariation
      μ hPrimary hSecondary with
    ⟨mean_choice, reflected, translation, C, R,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright,
      hboundary, hnormalized_atomized⟩
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_moment_zero_neighborhood_data
    (hComponentReplacementAutoTailReplacementRigidityMomentZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_normalized_atomization_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentReplacementAutoTailReplacementRigidityMomentZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨mean_choice, reflected, translation, C, R,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright,
      hboundary, hsecondMoment_eq, hzero⟩
  have hcomponent_open : IsOpen C.interval := by
    rw [hcomponent_interval]
    exact isOpen_Ioo
  have hunique :
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_component_zero_neighborhood_data
    (hComponentReplacementAutoTailReplacementRigidityComponentZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentReplacementAutoTailReplacementRigidityComponentZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, R, xMinus, xPlus, hcomponent_interval, hbaseline, hright,
      hboundary, hzero⟩
  exact
    ⟨TaoVariationMeanChoice.nonnegativeMean, false, 0, C, R,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright, hboundary,
      hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_mass_positive_zero_neighborhood_data
    (hComponentMassPositiveZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_replacement_auto_tail_replacement_rigidity_component_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentMassPositiveZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, xMinus, xPlus, hmass_pos, hcomponent_interval, hbaseline, hright,
      hboundary, hzero⟩
  exact
    ⟨C, ComponentReplacement.of_mass_pos C hmass_pos,
      xMinus, xPlus, hcomponent_interval, hbaseline, hright, hboundary,
      hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_intrinsic_mass_positive_zero_neighborhood_data
    (hComponentIntrinsicMassPositiveZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_mass_positive_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentIntrinsicMassPositiveZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_endpoint_atom_zero_neighborhood_data
    (hComponentEndpointAtomZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_support_point_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hComponentEndpointAtomZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, hendpoint_pos, hendpoint_mem, hbaseline, hright, hboundary, hzero⟩
  exact
    ⟨C, -1,
      realMeasure_mem_support_of_singleton_pos hendpoint_pos,
      hendpoint_mem, hbaseline, hright, hboundary, hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_zero_neighborhood_data
    (hAugmentedMaximalComponentZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_endpoint_atom_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_real_pos, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hzero⟩
...
/--
Augmented-left-neighbourhood version of
`...augmented_maximal_component_zero_neighborhood_data`.  This is the
pole-as-win interface: the component-selection input may live in the augmented
positive set, while the endpoint atom still supplies the actual support point
needed by the endpoint package.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_augmented_left_zero_neighborhood_data
    (hAugmentedMaximalComponentAugmentedLeftZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_component_endpoint_atom_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentAugmentedLeftZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_aug, hendpoint_real_pos,
...
/--
Subtype-endpoint-atom version of the augmented-left endpoint provider.  The
real push-forward endpoint atom is derived mechanically from the subtype atom,
so the variation input only has to expose the actual normalized endpoint atom.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_augmented_left_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_aug, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hzero⟩
  exact
    ⟨C, ε, hε, hmax, hleft_aug,
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAtomizationDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hcomponent_atomized⟩
  exact
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hright, hboundary,
      component_neighborhood_zero_of_componentBlock_eq_smul_dirac_endpoint
        hcomponent_atomized⟩

/--
Augmented-left version of the component-atomization provider.  Component block
atomization supplies the zero-neighbourhood condition, while the left
component-selection input may remain in the augmented positive set.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftAtomizationDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_aug, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hcomponent_atomized⟩
  exact
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_normalized_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomNormalizedAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_atomization_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomNormalizedAtomizationDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hnormalized_atomized⟩
...
/--
Augmented-left version of the normalized-component atomization provider.  This
keeps the component-selection input in the augmented positive set and converts
`normalizedComponentBlock C = dirac (-1)` to the scaled component-block
atomization internally.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_normalized_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftNormalizedAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_atomization_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAugmentedLeftNormalizedAtomizationDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_aug, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hnormalized_atomized⟩
  have hendpoint_real_pos :
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_moment_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hright, hboundary, hzero⟩
  have hendpoint_real_pos :
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_positive_point_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomPositivePointReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomPositivePointReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xPos, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hxPos_pos, hxPos_mem, hboundary, hzero⟩
  exact
    ⟨C, ε, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline,
      C.right_pos_of_pos_mem_interval hxPos_pos hxPos_mem,
      hboundary, hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_augmented_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomRightAugmentedReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
        ...
        ∃ ε xRight : ℝ,
          ...
          0 < (μ : Measure UnitInterval1038)
            {t : UnitInterval1038 | (t : ℝ) = -1} ∧
          Set.Ioo (-1 : ℝ) 0 ⊆ C.interval ∧
          Set.Ioo (-1 : ℝ) xRight ⊆ unitIntervalAugmentedPositiveSet μ ∧
          1 ≤ (C.right + 1) *
              (((μ : Measure UnitInterval1038)
                {t : UnitInterval1038 | (t : ℝ) = -1}).toReal) +
            (1 - C.right) *
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_positive_point_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomRightAugmentedReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hright_aug, hboundary, hzero⟩
  let xPos : ℝ := xRight / 2
  have hxPos_pos : 0 < xPos := by
    dsimp [xPos]
    linarith
  have hxPos_interval : xPos ∈ C.interval := by
    have hJ_inter :
        (Set.Ioo (-1 : ℝ) xRight ∩ C.interval).Nonempty := by
      refine ⟨-(1 : ℝ) / 2, ?_⟩
      constructor
      · constructor
        · norm_num
        · linarith
      · exact hbaseline (by norm_num)
    have hJ_subset : Set.Ioo (-1 : ℝ) xRight ⊆ C.interval :=
      hmax (-1 : ℝ) xRight (by linarith) hright_aug hJ_inter
    exact hJ_subset (by
      constructor
      · dsimp [xPos]
        linarith
  ...
  exact
    ⟨C, ε, xPos, hε, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hxPos_pos, hxPos_interval, hboundary, hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_positive_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomRightPositiveReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
        ...
        ∃ ε xRight : ℝ,
          ...
          0 < (μ : Measure UnitInterval1038)
            {t : UnitInterval1038 | (t : ℝ) = -1} ∧
          Set.Ioo (-1 : ℝ) 0 ⊆ C.interval ∧
          (0 : ℝ) ∈ unitIntervalAugmentedPositiveSet μ ∧
          Set.Ioo (0 : ℝ) xRight ⊆
            PositiveSet (unitIntervalLogPotential μ) ∧
          1 ≤ (C.right + 1) *
              (((μ : Measure UnitInterval1038)
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_augmented_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomRightPositiveReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hzero_aug, hright_pos, hboundary, hzero⟩
  have hright_aug :
      Set.Ioo (-1 : ℝ) xRight ⊆ unitIntervalAugmentedPositiveSet μ := by
    intro x hx
    by_cases hx_neg : x < 0
    · exact unitInterval_positiveSet_subset_augmented μ
        (C.interval_subset_positiveSet (hbaseline ⟨hx.1, hx_neg⟩))
    · have hx_nonneg : 0 ≤ x := le_of_not_gt hx_neg
      by_cases hx_zero : x = 0
      · simpa [hx_zero] using hzero_aug
      · have hx_pos : 0 < x := lt_of_le_of_ne hx_nonneg (Ne.symm hx_zero)
        exact unitInterval_positiveSet_subset_augmented μ
          (hright_pos ⟨hx_pos, hx.2⟩)
  exact
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos,
      hendpoint_unit_pos, hbaseline, hright_aug, hboundary, hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_positive_zero_positive_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomRightPositiveZeroPositiveReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
        ...
        ∃ ε xRight : ℝ,
          ...
          0 < (μ : Measure UnitInterval1038)
            {t : UnitInterval1038 | (t : ℝ) = -1} ∧
          Set.Ioo (-1 : ℝ) 0 ⊆ C.interval ∧
          0 < unitIntervalLogPotential μ 0 ∧
          Set.Ioo (0 : ℝ) xRight ⊆
            PositiveSet (unitIntervalLogPotential μ) ∧
          1 ≤ (C.right + 1) *
              (((μ : Measure UnitInterval1038)
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_positive_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomRightPositiveZeroPositiveReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos, hendpoint_unit_pos,
      hbaseline, hzero_pos, hright_pos, hboundary, hzero⟩
  have hzero_aug : (0 : ℝ) ∈ unitIntervalAugmentedPositiveSet μ :=
    unitInterval_positiveSet_subset_augmented μ hzero_pos
  exact
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos,
      hendpoint_unit_pos, hbaseline, hzero_aug, hright_pos, hboundary,
      hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_local_positive_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomLocalPositiveReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_right_positive_zero_positive_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomLocalPositiveReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, δ, xRight, hε, hδ, hxRight_pos, hmax, hleft_pos,
      hendpoint_unit_pos, hbaseline, hlocal_pos, hboundary, hzero⟩
  have hzero_pos : 0 < unitIntervalLogPotential μ 0 :=
    hlocal_pos (by
      constructor
      · linarith
      · exact hxRight_pos)
  have hright_pos :
      Set.Ioo (0 : ℝ) xRight ⊆
        PositiveSet (unitIntervalLogPotential μ) := by
    intro x hx
    exact hlocal_pos ⟨by linarith [hδ, hx.1], hx.2⟩
  exact
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hleft_pos,
      hendpoint_unit_pos, hbaseline, hzero_pos, hright_pos, hboundary,
      hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_spanning_positive_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomSpanningPositiveReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_local_positive_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomSpanningPositiveReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xRight, hε, hxRight_pos, hmax, hspan_pos,
      hendpoint_unit_pos, hbaseline, hboundary, hzero⟩
  exact
    ⟨C, ε, 1, xRight, hε, by norm_num, hxRight_pos, hmax,
      (fun x hx => hspan_pos ⟨hx.1, by linarith [hx.2, hxRight_pos]⟩),
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_spanning_positive_boundary_log_distance_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomSpanningPositiveBoundaryLogDistanceZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_spanning_positive_boundary_distance_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomSpanningPositiveBoundaryLogDistanceZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, xRight, boundarySep, hε, hxRight_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
      hlog_int, hpotential_nonpos, hdistance_upper, hzero⟩
  exact
    ⟨C, ε, xRight, boundarySep, hε, hxRight_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_separated_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundarySeparatedZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_spanning_positive_boundary_separated_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundarySeparatedZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, boundarySep, hε, hright_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
      hpotential_nonpos, hdistance_upper, hzero⟩
  exact
    ⟨C, ε, C.right, boundarySep, hε, hright_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
      hpotential_nonpos, hdistance_upper, hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_not_positive_separated_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryNotPositiveSeparatedZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_separated_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryNotPositiveSeparatedZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, boundarySep, hε, hright_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
  ...
  have hpotential_nonpos : unitIntervalLogPotential μ C.right ≤ 0 := by
    exact le_of_not_gt (by
      intro hpos
      exact hright_not_pos (by simpa [PositiveSet] using hpos))
  exact
    ⟨C, ε, boundarySep, hε, hright_pos, hboundarySep,
      hmax, hspan_pos, hendpoint_unit_pos, hbaseline, hdist_lower,
      hpotential_nonpos, hdistance_upper, hzero⟩
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_excluded_data
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_not_augmented_not_support_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryExcludedDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, hε, hright_pos, hmax, hspan_pos,
  ...
  have hright_not_support : C.right ∉ (realMeasure μ).support := by
    intro hmem
    exact hright_excluded (Or.inr hmem)
  exact
    ⟨C, ε, hε, hright_pos, hmax, hspan_pos,
      hendpoint_unit_pos, hbaseline, hright_not_aug, hright_not_support,
      hzero⟩

theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_right_gap_data
    (hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryRightGapDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_excluded_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryRightGapDataFromVariation
      μ hPrimary hSecondary with
...
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_augmented_span_augmented_gap_not_support_data
    (hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryAugmentedSpanAugmentedGapNotSupportDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_intrinsic_boundary_augmented_span_right_gap_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomIntrinsicBoundaryAugmentedSpanAugmentedGapNotSupportDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, δ₀, hε, hright_pos, hδ₀, hmax, hspan_aug,
...
/--
Direct augmented-span/right-gap provider.

Unlike the previous wrapper, this route does not first convert the whole
augmented span into an ordinary positive span.  The right gap supplies both
`C.right ∉ unitIntervalAugmentedPositiveSet μ` and
`C.right ∉ (realMeasure μ).support`; these give the boundary nonpositive and
separation inputs.  The distance upper bound is derived directly from the
augmented span and the zero-neighbourhood condition.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_span_right_gap_boundary_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanRightGapDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
  ...
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_left_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanRightGapDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, δ, hε, hright_pos, hδ, hmax, hspan_aug,
      hendpoint_unit_pos, hbaseline, hright_gap, hzero⟩
  have hboundary :
  ...
  exact
    ⟨C, ε, hε, hmax, hleft_aug, hendpoint_unit_pos,
      hbaseline, hright_pos, hboundary, hzero⟩

/--
Augmented-span/right-gap provider with component-block atomization instead of
an explicit zero-neighbourhood hypothesis.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_span_right_gap_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanRightGapAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
...
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_span_right_gap_normalized_atomization_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanRightGapNormalizedAtomizationDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
...
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_span_right_gap_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanRightGapReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_unit_endpoint_atom_augmented_span_right_gap_replacement_rigidity_zero_neighborhood_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentUnitEndpointAtomAugmentedSpanAugmentedGapNotSupportReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, ε, δ₀, hε, hright_pos, hδ₀, hmax, hspan_aug,
      hendpoint_unit_pos, hbaseline, haug_gap, hnot_support, hzero⟩
  rcases C.right_gap_union_of_augmented_gap_not_mem_support
...
rigidity provider.  Compared with the previous theorem, the endpoint atom is
not an input: it is obtained after replacement rigidity gives normalized
endpoint atomization.
-/
theorem unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_replacement_augmented_span_right_gap_replacement_rigidity_zero_neighborhood_data
    (hAugmentedMaximalComponentReplacementAugmentedSpanRightGapReplacementRigidityZeroNeighborhoodDataFromVariation :
      ∀ μ : ProbabilityMeasure UnitInterval1038,
        (∀ ν : ProbabilityMeasure UnitInterval1038,
          unitIntervalTruncatedPositiveSetObjective μ ≤
            unitIntervalTruncatedPositiveSetObjective ν) →
    ...
    ∃ μ : ProbabilityMeasure UnitInterval1038,
      ...
      ∃ _hEndpoint : NormalizedEndpointPotential (unitIntervalLogPotential μ),
        ENNReal.ofReal (Real.sqrt 2) ≤
          volume (PositiveSet (unitIntervalLogPotential μ)) := by
  refine
    unitIntervalTruncatedPositiveSetObjective_exists_secondMoment_normalized_endpoint_baseline_from_augmented_maximal_component_replacement_augmented_span_right_gap_normalized_atomization_data
      ?_
  intro μ hPrimary hSecondary
  rcases hAugmentedMaximalComponentReplacementAugmentedSpanRightGapReplacementRigidityZeroNeighborhoodDataFromVariation
      μ hPrimary hSecondary with
    ⟨C, R, ε, δ, hε, hright_pos, hδ, hmax, hspan_aug,
