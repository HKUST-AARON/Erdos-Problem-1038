#!/usr/bin/env python3
"""Generate Lean rational arithmetic chunks for the forcing_1836 JSON certificate."""
...
CERT = ROOT / "forcing_1836_interval_certificate.json"
LEAN = ROOT / "lean"
CHUNKS = LEAN / "box_arith_chunks"
INDEX = LEAN / "Forcing1836BoxData.lean"
CHUNK_SIZE = 50
DEN = Decimal(10) ** 12
...
def write_chunk(idx: int, boxes: list[dict], start: int, end: int) -> None:
    ns = f"Forcing1836BoxArith{idx:03d}"
...
def main() -> None:
    with localcontext() as ctx:
        ...
        for old in CHUNKS.glob("Forcing1836BoxArith*.lean"):
            old.unlink()
    ...
    for idx, start in enumerate(range(0, len(enriched), CHUNK_SIZE)):
        ...
        chunk_names.append(f"Forcing1836BoxArith{idx:03d}")
